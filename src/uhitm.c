    ...
    if (mtmp->mundetected && !canseemon(mtmp)
        ...
        if (!tp_sensemon(mtmp) && !Detect_monsters) {
            ...
            if (!Blind && Hallucination)
                #ifdef ZHLANG
                pline("一只%s%s%s！", mtmp->mtame ? "tame" : "wild",
    ...
    struct obj *obj)    /* obj is not NULL */
{
    ...
    case MIRROR:
        if (breaktest(obj)) {
            #ifdef ZHLANG
            You("打碎了%s。真是不走运！", ysimple_name(obj));
#else
...
#define useup_eggs(o)                    \
    ...
    case EGG: {
        ...
        if (ismnum(obj->corpsenm)
            ...
            #ifdef ZHLANG
            You("用%s鸡蛋%s击中了%s。", mon_nam(mon), eggp,
                plur(cnt));
#else
    ...
    struct obj *obj) /* lance; obj is not NULL */
{
    ...
    #ifdef ZHLANG
    You("冲刺了%s%s", mon_nam(mon), canseemon(mon) ? exclam(hmd->dmg) : ".");
#else
    ...
    struct obj *obj)    /* obj can be NULL but guards are in place below */
{
    if ((hmd->mdat == &mons[PM_BLACK_PUDDING]
        ...
        if ((mclone = clone_mon(mon, 0, 0)) != 0) {
            ...
            #ifdef ZHLANG
            pline("%s被你击中时分裂了%s！", Monnam(mon), withwhat);
#else
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    ...
    if (magr == &gy.youmonst) {
        ...
        if (!Blind)
            #ifdef ZHLANG
            pline("%s%s了！", Monnam(mdef), on_fire(pd, mattk));
#else
    ...
    if (magr == &gy.youmonst) {
        ...
        if (!magr->mcan && !rn2(3))
            if (Acid_resistance) {
                ...
                #ifdef ZHLANG
                pline("你被%s覆盖了！它在灼烧！", hliquid("acid"));
#else
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    ...
    if (magr == &gy.youmonst) {
        ...
        if (gv.vis && canseemon(mdef)) {
            #ifdef ZHLANG
            pline("%s从%s那里偷走了一些金币。", buf, mon_nam(mdef));
#else
                    ...
                    struct monst *mdef, struct mhitm_data *mhm)
{
    if (gv.vis && canspotmon(magr))
        #ifdef ZHLANG
        pline("%s的%s被下毒了！", s_suffix(Monnam(magr)),
    ...
    if (magr == &gy.youmonst) {
        ...
        if (mdef->mcanmove && !rn2(3)
            ...
            if (gv.vis && canspotmon(mdef)) {
                ...
                #ifdef ZHLANG
                pline("%s被%s冻住了。", buf, mon_nam(magr));
#else
    ...
    if (magr == &gy.youmonst) {
        ...
        if (!negated) {
            ...
            if (obj && drain_item(obj, FALSE)) {
                #ifdef ZHLANG
                pline("%s效果减弱了。", Yobjnam2(obj, "seem"));
#else
    ...
    if (magr == &gy.youmonst) {
        ...
        if ((u.usteed || Levitation || Flying) && !is_flyer(magr->data)) {
            ...
            if (uarmf) {
                ...
                #ifdef ZHLANG
                pline("%s刺到了你的%s%s！", Monst_name, sidestr, leg);
#else
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    ...
    if (magr == &gy.youmonst) {
        ...
        if (obj) {
            ...
            if (gv.vis && canseemon(mdef)) {
                #ifdef ZHLANG
                pline("%s从%s那里偷走了%s！", buf, onambuf, mdefnambuf);
#else
...
gulpum(struct monst *mdef, struct attack *mattk)
{
    ...
    if (!(u_digest && u.uhunger >= 1500) && !u.uswallow) {
        ...
        if (fatal_gulp && !is_rider(pd)) { /* petrification */
            ...
            case AD_ACID:
                #ifdef ZHLANG
                pline("%s被你的黏液覆盖了！", Monnam(mdef));
#else
                pline("%s is covered with your goo!", Monnam(mdef));
#endif
                if (resists_acid(mdef)) {
                    #ifdef ZHLANG
                    pline("对%s似乎无害。", mon_nam(mdef));
#else
    ...
    boolean weapon_used)  /* True: via weapon hit */
{
    ...
    if (u_def || canseemon(mdef)) {
        ...
        #ifdef ZHLANG
        You_feel("感觉%s被%s了！", some_mon_nam(mdef), knockedhow);
#else
            ...
            if (ptr == &mons[PM_FLOATING_EYE]) {
                ...
                if (mon->mcansee) {
                    if (ureflects("%s gaze is reflected by your %s.",
                        ...
                        #ifdef ZHLANG
                        You("被%s的凝视冻住了！", s_suffix(mon_nam(mon)));
#else
