    ...
    unsigned trflags)
{
    ...
    if (is_you) {
        ...
        if (u.usteed) {
            #ifdef ZHLANG
            pline("%s捕熊夹夹住了%s的%s！", A_Your[trap->madeby_u],
            ...
            #ifdef ZHLANG
            pline("%s捕熊夹夹住了你的%s！", A_Your[trap->madeby_u],
                  body_part(FOOT));
#else
    ...
    unsigned int trflags UNUSED)
{
    ...
    if (mtmp == &gy.youmonst) {
        ...
        case 0:
            #ifdef ZHLANG
            pline("%s击中了你的%s！", A_gush_of_water_hits, body_part(HEAD));
#else
        ...
        case 1:
            #ifdef ZHLANG
            pline("%s击中了你的左%s！", A_gush_of_water_hits, body_part(ARM));
#else
 ...
 uglovecheck:
        ...
        case 2:
            #ifdef ZHLANG
            pline("%s击中了你的右%s！", A_gush_of_water_hits, body_part(ARM));
#else
        ...
        default:
            #ifdef ZHLANG
            pline("%s击中了你！", A_gush_of_water_hits);
#else
    ...
    if (mtmp == &gy.youmonst) {
        ...
        if (!Sokoban && is_clinger(gy.youmonst.data) && !plunged) {
            if (already_known) {
                ...
                #ifdef ZHLANG
                pline("%s陷阱坑%s在你脚下打开！", A_Your[trap->madeby_u],
        ...
        if (!Sokoban) {
            ...
            *verbbuf = '\0';
            ...
            if (*verbbuf)
                #ifdef ZHLANG
                You("%s进了%s陷阱坑！", verbbuf, a_your[trap->madeby_u]);
#else
    ...
    unsigned int trflags)
{
    if (mtmp == &gy.youmonst) {
        ...
        if (webmaker(gy.youmonst.data)) {
            if (webmsgok)
                #ifdef ZHLANG
                pline(trap->madeby_u ? "你在自己的网上散了个步。"
                      : "这里有一张蜘蛛网。");
#else
        ...
        if (webmsgok) {
            ...
            #ifdef ZHLANG
            You("%s了%s蜘蛛网！", verbbuf, a_your[trap->madeby_u]);
#else
        ...
        /* Time stuck in the web depends on your/steed's strength. */
        {
            ...
            if (str <= 3)
                ...
                if (webmsgok)
                    #ifdef ZHLANG
                    You("撕破了%s网！", a_your[trap->madeby_u]);
#else
    ...
    if (wearing_iron_shoes(mtmp)) {
        ...
        if (shoes->spe > 0) {
            ...
            if (mtmp == &gy.youmonst) {
                seetrap(trap);
                #ifdef ZHLANG
                pline("一股倦怠的气息环绕着%s。", yname(shoes));
#else
...
dotrap(struct trap *trap, unsigned trflags)
{
    ...
    if (Sokoban && (is_pit(ttype) || is_hole(ttype))) {
        ...
        pline("Air currents pull you down into %s %s!",
              a_your[trap->madeby_u],
              trapname(ttype, TRUE))
#endif /* do force "pit" while hallucinating */
        /* then proceed to normal trap effect */
    } else if (!forcetrap) {
        if (floor_trigger(ttype) && check_in_air(&gy.youmonst, trflags)) {
            if (already_seen) {
                #ifdef ZHLANG
                You("%s%s%s。", u_locomotion("step"),
...
selftouch(const char *arg)
{
    ...
    if (uwep && uwep->otyp == CORPSE && touch_petrifies(&mons[uwep->corpsenm])
        ...
        #ifdef ZHLANG
        pline("%s触碰了%s的尸体。", arg, corpse_pmname);
#else
...
float_up(void)
{
    ...
    if (u.utrap) {
        if (u.utraptype == TT_PIT) {
            reset_utrap(FALSE);
            #ifdef ZHLANG
            You("漂浮起来，离开了%s！", trapname(PIT, FALSE));
#else
        ...
        if (is_animal(u.ustuck->data))
            #ifdef ZHLANG
            You("飘离了%s。", surface(u.ux, u.uy));
#else
            You("float away from the %s.", surface(u.ux, u.uy));
#endif
        else
            #ifdef ZHLANG
            You("螺旋上升进了%s。", mon_nam(u.ustuck));
#else
    ...
    if (u.usteed && !is_floater(u.usteed->data)
        ...
        if (Lev_at_will) {
            #ifdef ZHLANG
            pline("%s魔法般地飘起来了！", Monnam(u.usteed));
#else
            pline("%s magically floats up!", Monnam(u.usteed));
#endif
        } else {
            #ifdef ZHLANG
            You("无法继续骑在%s上。", mon_nam(u.usteed));
#else
...
climb_pit(void)
{
    ...
    if (Passes_walls) {
        ...
        #ifdef ZHLANG
        You("%s出了%s。", u_locomotion("climb"), pitname);
#else
        ...
        if (u.usteed)
            #ifdef ZHLANG
            Norep("%s还在陷阱坑里。", YMonnam(u.usteed));
#else
    ...
    struct obj *box) /* null for floor trap */
{
    ...
    #ifdef ZHLANG
    pline("一道%s从%s%s！", tower_of_flame, box ? "bursts" : "erupts",
          the(box ? xname(box) : surface(u.ux, u.uy)));
#else
...
domagictrap(void)
{
    ...
    if (fate < 10) {
        ...
        case 13:
            #ifdef ZHLANG
            pline("一股战栗沿着你的%s上下窜动！", body_part(SPINE));
#else
        ...
        case 17:
            #ifdef ZHLANG
            You(Hallucination ? "smell hamburgers." : "闻到了烧焦的肉味。");
#else
    ...
    if (objects[otyp].oc_material < DRAGON_HIDE
        ...
        if (cansee(x, y)) {
            ...
            if (obj == gt.thrownobj || obj == gk.kickedobj)
                #ifdef ZHLANG
                pline("%s%s烧起来了！", is_plural(obj) ? "They" : "It",
                      otense(obj, "burn"));
#else
...
drown(void)
{
    ...
    if (!u.uinwater) {
        ...
        if (!Swimming && !is_solid)
            #ifdef ZHLANG
            You("像%s一样沉了下去。", Hallucination ? "the Titanic" : "a rock");
#else
...
disarm_holdingtrap(struct trap *ttmp)
{
    ...
    if ((mtmp = m_at(ttmp->tx, ttmp->ty)) != 0) {
        mtmp->mtrapped = 0;
        #ifdef ZHLANG
        You("把%s从%s%s中救了出来。", mon_nam(mtmp),
        ...
        if (wep && wep->oartifact
            ...
            #ifdef ZHLANG
            pline("%s%s了%s网！", bare_artifactname(uwep),
...
disarm_landmine(struct trap *ttmp) /* Helge Hafting */
{
    ...
    if (fails < 2)
        return fails;
    #ifdef ZHLANG
    You("拆除了%s地雷。", the_your[ttmp->madeby_u]);
#else
...
disarm_shooting_trap(struct trap *ttmp, int otyp)
{
    ...
    if (fails < 2)
        return fails;
    #ifdef ZHLANG
    You("拆除了%s陷阱。", the_your[ttmp->madeby_u]);
#else
    ...
    struct trap *ttmp)
{
    ...
    if (uprob) {
        #ifdef ZHLANG
        You("试着抓住%s，但无法牢牢抓住。", mon_nam(mtmp));
#else
    ...
    #ifdef ZHLANG
    You("伸出你的%s抓住了%s。", makeplural(body_part(ARM)),
        mon_nam(mtmp));
#else
    ...
    boolean confused)
{
    if ((box->otrapped
        ...
        if (!(box->tknown && box->dknown))
            #ifdef ZHLANG
            You("发现%s上有陷阱！", the(xname(box)));
#else
            You("find a trap on %s!", the(xname(box)));
#endif
        else
            #ifdef ZHLANG
            pline("%s上有陷阱。", the(xname(box)));
#else
                       ...
                       * otherwise left with its previous value intact */
{
    ...
    if (ishero) {
        ...
        *noticed = TRUE;
        ...
        #ifdef ZHLANG
        pline("%s从%s%s释放了出来。", buf, which, trapdescr);
#else
        ...
        if (canspotmon(mon)) {
            *noticed = TRUE;
            #ifdef ZHLANG
            pline("%s从%s%s释放了出来。", Monnam(mon), which,
                  trapdescr);
#else
            ...
            *noticed = TRUE;
            if (t->ttyp == WEB)
                #ifdef ZHLANG
                pline("%s从%s%s释放了出来。", Something, which,
                      trapdescr);
#else
    ...
    boolean disarm)
{
    ...
    if (Luck > -13 && rn2(13 + Luck) > 7) { /* saved by luck */
        ...
        case 17:
            #ifdef ZHLANG
            pline("一团有毒气体从%s中涌出。", the(xname(obj)));
#else
        ...
        case 13:
            #ifdef ZHLANG
            You_feel("一根针刺了你的%s。", body_part(bodypart));
#else
...
lava_effects(void)
{
    ...
    if (uarmf && (uarmf->in_use
        ...
        obj = uarmf;
        #ifdef ZHLANG
        pline("%s烧了起来！", Yobjnam2(obj, "burst"));
#else
    ...
    if (!Fire_resistance) {
        ...
        set_utrap((unsigned) (rn1(4, 4) + ((boil_away ? 2
                  ...
                  TT_LAVA);
        #ifdef ZHLANG
        You("沉入了%s%s！", waterbody_name(u.ux, u.uy),
